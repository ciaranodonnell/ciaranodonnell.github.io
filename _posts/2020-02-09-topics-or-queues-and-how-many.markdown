...
It's increasingly common for our microservices style systems to be based around [Message Brokers](https://en.wikipedia.org/wiki/Message_broker) and their ability to transfer messages, little packets of data, for us.
...
A [Queue](https://en.wikipedia.org/wiki/Message_queue) is a message brokers implementation of the basic Computer Science data structure called a [Queue](https://en.wikipedia.org/wiki/Queue_(abstract_data_type)).
...
[Competing Consumers](https://www.enterpriseintegrationpatterns.com/patterns/messaging/CompetingConsumers.html) is a situation when two consumers connect to the same queue to receive messages.
...
A Topic is different from a Queue. In fact a Topic is always paired with the idea of a subscription.
